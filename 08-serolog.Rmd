...
```{r}
modx <- lrm(infection_viv ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
#modx <- lrm(as.factor(pheno.num) ~ igg + igg1 + igg2 + igg3 + rms::rcs(igg4,3) + rms::rcs(par,3) + edad + sexo, data=mmo_x_rms, x=TRUE, y=TRUE)
modx
paste("AIC=",AIC(modx))
```
...
```{r}
#summary(mod1)
#Hmisc::html(summary(mod1),file="",dec=2)
```

```{r}
#ggplot2::ggplot(rms::Predict(mod1))
#ggplot2::ggplot(rms::Predict(mod1,fun=plogis))
```
...
```{r}
my.valid <- rms::validate(modx, method="boot", B=1000)
my.valid
```
...
```{r,fig.height=3,fig.width=6}
my.calib <- rms::calibrate(modx, method="boot", B=1000)
par(bg="white", las=1)
plot(my.calib, las=1)
```
...
```{r}
penalty <- rms::pentrace(modx, penalty=seq(-50,50,by=.25), maxit=25)
#penalty
modx_pen <- update(modx, penalty=penalty$penalty)
paste("AIC=",AIC(modx_pen))
modx_pen
#rms::effective.df(mod1_pen)
#mod1_pen
```
...
#options(prType='latex')
#summary(modx_pen)
Hmisc::html(summary(modx_pen),file="",dec=2)
```

```{r}
#ggplot2::ggplot(rms::Predict(modx_pen))
ggplot2::ggplot(rms::Predict(modx_pen,fun=plogis))
```

```{r}
plot(summary(modx_pen), log=TRUE,main = "log Odds Ratio",cex.main = 0)
```

##### validity

```{r}
my.valid <- rms::validate(modx_pen, method="boot", B=1000)
my.valid
```
...
```{r,fig.height=3,fig.width=6}
my.calib <- rms::calibrate(modx_pen, method="boot", B=1000)
par(bg="white", las=1)
plot(my.calib, las=1)
```
...
```{r}
lrm.full <- lrm(infection_viv ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra,x=TRUE, y=TRUE)
#stargazer::stargazer(lrm.full)
paste("AIC=",AIC(lrm.full))
#lrm.full
rms::fastbw(lrm.full, rule = "p", sls = 0.5)
```

##### selected model

```{r}
lrm.finl <- lrm(infection_viv ~ sex_8 + community_1, data=unap10a_ra, x=TRUE, y=TRUE)
paste("AIC=",AIC(lrm.finl))
lrm.finl
#rms::fastbw(lrm.finl, rule = "p", sls = 0.5)
```
...
```{r}
my.valid <- rms::validate(mod1, method="boot", B=1000)
my.valid
```
...
```{r,fig.height=3,fig.width=6}
my.calib <- rms::calibrate(mod1, method="boot", B=1000)
par(bg="white", las=1)
plot(my.calib, las=1)
```
...
```{r}
#stargazer::stargazer(modx_pen,lrm.finl,type = "text")
#options(prType = "plain")

#print(modx_pen,long=F)
#summary(modx_pen)
#print(lrm.finl,long=F)
#summary(lrm.finl)

stargazer::stargazer(modx_pen,lrm.finl,
                     type = "text", apply.coef=exp, #apply.ci=exp,
                     t.auto=F, p.auto=F, report = "vcs*",ci = T,
                     #no.space = F,
                     single.row = T
                     )

lrm.full_iv <- modx_pen
lrm.finl_iv <- lrm.finl
```
...
```{r}
modx <- lrm(exposure_viv ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
#modx <- lrm(as.factor(pheno.num) ~ igg + igg1 + igg2 + igg3 + rms::rcs(igg4,3) + rms::rcs(par,3) + edad + sexo, data=mmo_x_rms, x=TRUE, y=TRUE)
modx
paste("AIC=",AIC(modx))
```
...
```{r,fig.height=3,fig.width=6}
plot(anova(mod1), what='proportion chisq') # relative importance
#plot(rms::Predict(mod1, fun=plogis)) # predicted values
```
...
```{r}
lrm.full <- lrm(exposure_viv ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
paste("AIC=",AIC(lrm.full))
#lrm.full
rms::fastbw(lrm.full, rule = "p", sls = 0.5)
```
...
```{r}
lrm.finl <- lrm(exposure_viv ~ age_7 + community_1 + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
paste("AIC=",AIC(lrm.finl))
lrm.finl
#rms::fastbw(lrm.finl, rule = "p", sls = 0.5)
```
...
lrm.full_ev <- modx_pen
lrm.finl_ev <- lrm.finl
```

### Pf infection

- 12 casos de 824 observaciones.
    - _overfitting_: 
...
```{r}
modx <- lrm(infection_fal ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared +  material_piso + epi_uso_red_dormir, data=unap10a_ra %>% filter(community_1!="Puerto Almendra"), x=TRUE, y=TRUE)
#modx <- lrm(as.factor(pheno.num) ~ igg + igg1 + igg2 + igg3 + rms::rcs(igg4,3) + rms::rcs(par,3) + edad + sexo, data=mmo_x_rms, x=TRUE, y=TRUE)
modx
paste("AIC=",AIC(modx))
```
...
```{r}
lrm.full <- lrm(infection_fal ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
paste("AIC=",AIC(lrm.full))
#lrm.full
rms::fastbw(lrm.full, rule = "p", sls = 0.5)
```
...
```{r}
lrm.finl <- lrm(infection_fal ~ electricidad_red_publica, data=unap10a_ra, x=TRUE, y=TRUE)
paste("AIC=",AIC(lrm.finl))
lrm.finl
#rms::fastbw(lrm.finl, rule = "p", sls = 0.5)
```
...
lrm.full_if <- modx_pen
lrm.finl_if <- lrm.finl
```
...
```{r}
modx <- lrm(exposure_fal ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
#modx <- lrm(as.factor(pheno.num) ~ igg + igg1 + igg2 + igg3 + rms::rcs(igg4,3) + rms::rcs(par,3) + edad + sexo, data=mmo_x_rms, x=TRUE, y=TRUE)
modx
paste("AIC=",AIC(modx))
```
...
```{r}
lrm.full <- lrm(exposure_fal ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
paste("AIC=",AIC(lrm.full))
#lrm.full
rms::fastbw(lrm.full, rule = "p", sls = 0.1)#changed default
```

##### selected model

```{r}
lrm.finl <- lrm(exposure_fal ~ sex_8 + age_7 + community_1 + trabajo_rpl + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
paste("AIC=",AIC(lrm.finl))
lrm.finl
#rms::fastbw(lrm.finl, rule = "p", sls = 0.5)
```
...
lrm.full_ef <- modx_pen
lrm.finl_ef <- lrm.finl
```

### Stargazer presentation

```{r}
#lrm.full_iv <- lrm(infection_viv ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
#lrm.full_ev <- lrm(exposure_viv ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
#lrm.finl_iv <- lrm(infection_viv ~ sex_8 + community_1, data=unap10a_ra, x=TRUE, y=TRUE)
#lrm.finl_ev <- lrm(exposure_viv ~ age_7 + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)

#lrm.full_if <- lrm(infection_fal ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
#lrm.full_ef <- lrm(exposure_fal ~ sex_8 + age_7 + community_1 + trabajo_rpl + electricidad_red_publica + material_pared + material_piso + epi_uso_red_dormir, data=unap10a_ra, x=TRUE, y=TRUE)
#lrm.finl_if <- lrm(infection_fal ~ electricidad_red_publica, data=unap10a_ra, x=TRUE, y=TRUE)
#lrm.finl_ef <- lrm(exposure_fal ~ age_7 + community_1 + material_pared, data=unap10a_ra, x=TRUE, y=TRUE)

#stargazer::stargazer(lrm.full_ev,lrm.finl_ev,
 #                    lrm.full_iv,lrm.finl_iv,
  #                   type = "latex", apply.coef=exp, #apply.ci=exp,
   #                  t.auto=F, p.auto=F, report = "vcs*",ci = T,
    #                 #no.space = F,
     #                single.row = T,
      #               title="Multivariate and backward selected model for P.vivax exposure and infection")

#stargazer::stargazer(lrm.full_ef,lrm.finl_ef,
 #                    lrm.full_if,lrm.finl_if,
  #                   type = "latex", apply.coef=exp, #apply.ci=exp,
   #                  t.auto=F, p.auto=F, report = "vcs*",ci = T,
    #                 #no.space = F,
     #                single.row = T,
      #               title="Multivariate and backward selected model for P.falciparum exposure and infection")

stargazer::stargazer(lrm.full_iv,lrm.finl_iv,
                     lrm.full_if,lrm.finl_if,
                     type = "latex", apply.coef=exp, #apply.ci=exp,
                     t.auto=F, p.auto=F, report = "vcs*",ci = T,
                     #no.space = F,
                     single.row = T,
                     title="Multivariate and backward selected model for species-specific malaria infection")

stargazer::stargazer(lrm.full_ev,lrm.finl_ev,
                     lrm.full_ef,lrm.finl_ef,
                     type = "latex", apply.coef=exp, #apply.ci=exp,
                     t.auto=F, p.auto=F, report = "vcs*",ci = T,
                     #no.space = F,
                     single.row = T,
                     title="Multivariate and backward selected model for species-specific malaria exposure")
...
unap10a_rc %>% 
  dplyr::select(community_1,age_7,#sum.sero.viv,
                tot.sero.viv,
                #sum.sero.fal,
                tot.sero.fal#,5,8
                ) %>% 
  gather(Specie,sero.t,-community_1,-age_7#,-sero.viv.p,-sero.fal.p
         ) %>% 
  ggplot(aes(age_7,sero.t)) +
  geom_point() +
  geom_hline(yintercept = 7, colour="red") +
  facet_grid(community_1~Specie)
```
...
unap10a_rc_v <- unap10a_rc %>% 
  dplyr::select(1:5,tot.sero.viv) %>% 
  filter(!(sero.viv.p==0 | sero.viv.p==1)) #%>% #glimpse()
  #filter(tot.sero.viv>=7) %>% dplyr::select(-tot.sero.viv)

unap10a_rc_f <- unap10a_rc %>% 
  dplyr::select(1,2,6:8,tot.sero.fal) %>% 
  filter(!(sero.fal.p==0 | sero.fal.p==1)) #%>% #dplyr::count(community_1)
  #filter(tot.sero.fal>=7) %>% dplyr::select(-tot.sero.fal)
