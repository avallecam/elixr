...
unap7 <- unap7_pr %>% #mejor cambialo a "_"
  select(id,sexo_corregido,edad_DNI,Pv_MSP1,Pf_MSP1,densidad=Densidad_1er)
```
...
unap8 <- unap8_ps %>% #mejor cambialo a "_"
  #select(id, contains("edad"), -enfermedad_cronica)
  #select(id, contains("sexo"))
  select(id, edad_DNI, residence, sexo_consentimiento) #%>% #summary()
```
...
unap8_ps %>% 
  ...
  select(id,household,electricidad_red_publica:otro_banio_conectado,
...
unap9c <- unap6c %>% 
  ...
  select(-age_1,-age_4,-edad_DNI.8,-sex_1,-sex_4,-sexo_corregido,-Pf_MSP1,-Pv_MSP1,-residence.6) %>% 
  dplyr::rename(age_7=edad_DNI.7,
...
unap9d <- unap8_ps.ty %>% inner_join(unap9c, by="id", suffix=c(".8",".9")) %>% #glimpse()
  select(-ends_with(".9")) %>% 
  rename_all(
      funs(
  ...
  select(id,sex_8,age_7,community_1,everything()) %>% 
...
saveRDS(unap9d, "data/05-epidemio-core.rds")
```
