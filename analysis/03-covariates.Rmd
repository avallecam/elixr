...
- `unap0` is in an untidy format
- `unap0` requires to be tidied before or after merging it with `std.nty`
- `unap0` can be directly merged with `std.uty`
...
std.nty.pre_Pv <- std.nty.pre %>% 
  #select(id, Specie, micro_1, micro_n, pcr_1, pcr_n) %>% 
...
- Validate the procedure contrasting this subset against `summary(std.uty.cov)`:
```{r}
#
std.nty.cov %>% 
  select(Specie, micro_1) %>% 
  group_by(Specie, micro_1) %>% 
  count()

std.nty.cov %>% 
  select(Specie, pcr_n) %>% 
  group_by(Specie, pcr_n) %>% 
  count()
