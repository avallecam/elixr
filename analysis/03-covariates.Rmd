...
- total amount of observations **`r length(std.nty$ID)`**
- total amount of evaluated patients **`r length(levels(std.nty$ID))`**
- giving **`r length(std.uty$ID)`** available samples in the `std.uty`.
...
g [label = '@@7', color= red, fontcolor= red]
h [label = '@@8', color= red, fontcolor= red]

# edge definitions with the node IDs
a -> {b c}
a -> {d e}  [color= red]
f -> a [arrowhead= none, color= green]
e -> {g h} [arrowhead= none, color= red]
}
...
#filter NA
epi <- epi[1:sum(epi$id_integrante != "", na.rm = T), 
           1:sum(colnames(epi) != "", na.rm= T)]
#963x109
#colnames(epi)
#str(epi)
```
...
- `std.uty.cov` output is write as `data/03-standard-untidy-cov.csv`.
- `std.nty.cov` output is write as `data/03-standard-tidy-cov.csv`:
