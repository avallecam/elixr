...
* TO DO:
    - In order to keep using `std.nty` as a tidy dataset 
        ...
        + More details in [APPENDIX F](#appendix-f-tidy-vs-untidy)
    - re-evaluate 01 patient 02 id after `unap6`
        + evaluation in [APPENDIX G](#appendix-g-id-errors)
...
- `unap0` was **merged** with both `std.nty` and `std.uty` DB of mean.OD and Ab.units per specie obtained in **02-data-filtering.Rmd**
...
- Validation in [APPENDIX F](#appendix-f-tidy-vs-untidy)
...
- given the mentioned assumption, percentage increases due to an updated logic variables.
    + Check details in [APPENDIX F](#appendix-f-tidy-vs-untidy)
...
# APPENDIX F: tidy vs untidy

## subset mixed infections
...
## Relevant for the QC evaluation 
