...
naMAT <- std.raw[is.na(std.raw$Ab.unit)==TRUE,]
#naMAT
```
...
# FINAL
#dim(std.raw)[1]
#esp - dim(std.raw)[1] # 55 ?

# CLEAN FINAL DATA.FRAME
std.raw$ID <- factor(std.raw$ID)
#length(levels(std.raw$ID))

#mue - length(levels(std.raw$ID)) == (length(NaN_2) -1) # 3 muestras que pierden Pviv y Pfal por NaN Ab.unit

#length(levels(std.raw$ID)) + (esp - dim(std.raw)[1])
```

After filtering, we obtain **`r length(levels(std.raw$ID))` samples** of **`r mue` expected**, giving a **total of `r dim(std.raw)[1]` reads** on the data frame.
...
```{r}
#CHECK
#sum(table(std.raw$ID)!=2) # 49 muestras que pierden a su par por NaN en Ab.units
#sum(table(std.raw$ID)!=2) + (length(NaN_2) -1)*2 == esp - dim(std.raw)[1] # 55 ?

#names(which(table(std.raw$ID)!=2))
```
...
my <- max(hist(std.raw$mean.OD, plot = F)$counts, na.rm=TRUE)

hist(std.raw$mean.OD, xlim = c(0,1.4), 
     main = "mean.OD Histogram", xlab = "mean.OD")
#plot(density(na.omit(std.raw$mean.OD)), 
#     main = "mean.OD Density Plot", xlab = "mean.OD")#, ylim = c(0,my)
#qqnorm(std.raw$mean.OD); qqline(std.raw$mean.OD)

hist(std.raw[std.raw$Specie=="Pviv",]$mean.OD, ylim = c(0,my), xlim = c(0,1.4), 
     main = "P.vivax", xlab = "mean.OD")
#plot(density(na.omit(std.raw[std.raw$Specie=="Pviv",]$mean.OD)),
#     main = "P.vivax", xlab = "mean.OD")#, ylim = c(0,my)
#qqnorm(std.raw[std.raw$Specie=="Pviv",]$mean.OD); qqline(std.raw[std.raw$Specie=="Pviv",]$mean.OD)

hist(std.raw[std.raw$Specie=="Pfal",]$mean.OD, ylim = c(0,my), xlim = c(0,1.4),
     main = "P.falciparum", xlab = "mean.OD")
#plot(density(na.omit(std.raw[std.raw$Specie=="Pfal",]$mean.OD)),
#     main = "P.falciparum", xlab = "mean.OD")#, ylim = c(0,my)
#qqnorm(std.raw[std.raw$Specie=="Pfal",]$mean.OD); qqline(std.raw[std.raw$Specie=="Pfal",]$mean.OD)
...
my <- max(hist(std.raw$Ab.unit, plot = F)$counts, na.rm=TRUE)

hist(std.raw$Ab.unit, 
     main = "Ab.units Histogram", xlab = "Ab.units")#, xlim = c(0,15000)
#plot(density(na.omit(std.raw$Ab.unit)), 
#     main = "Ab.units Density Plot", xlab = "Ab.units")#, ylim = c(0,my)
#qqnorm(std.raw$Ab.unit); qqline(std.raw$Ab.unit)

hist(std.raw[std.raw$Specie=="Pviv",]$Ab.unit, ylim = c(0,my),
     main = "P.vivax", xlab = "Ab.units")#, xlim = c(0,15000)
#plot(density(na.omit(std.raw[std.raw$Specie=="Pviv",]$Ab.unit)),
#     main = "P.vivax", xlab = "Ab.units")#, ylim = c(0,my)
#qqnorm(std.raw[std.raw$Specie=="Pviv",]$Ab.unit); qqline(std.raw[std.raw$Specie=="Pviv",]$Ab.unit)

hist(std.raw[std.raw$Specie=="Pfal",]$Ab.unit, ylim = c(0,my),
     main = "P.falciparum", xlab = "Ab.units")#, xlim = c(0,15000)
#plot(density(na.omit(std.raw[std.raw$Specie=="Pfal",]$Ab.unit)),
#     main = "P.falciparum", xlab = "Ab.units")#, ylim = c(0,my)
#qqnorm(std.raw[std.raw$Specie=="Pfal",]$Ab.unit); qqline(std.raw[std.raw$Specie=="Pfal",]$Ab.unit)
```
...
my <- max(hist(log(std.raw$mean.OD), plot = F)$counts, na.rm=TRUE)

hist(log(std.raw$mean.OD), xlim = c(-3.5,1),
     main = "mean.OD Histogram", xlab = "log(mean.OD)")
hist(log(std.raw[std.raw$Specie=="Pviv",]$mean.OD), ylim = c(0,my), xlim = c(-3.5,1),
     main = "P.vivax", xlab = "log(mean.OD)")
hist(log(std.raw[std.raw$Specie=="Pfal",]$mean.OD), ylim = c(0,my), xlim = c(-3.5,1),
     main = "P.falciparum", xlab = "log(mean.OD)")

############
#par(mfrow=c(1,3))

my <- max(hist(log(std.raw$Ab.unit), plot = F)$counts, na.rm=TRUE)

hist(log(std.raw$Ab.unit), xlim = c(-6,10),
     main = "Ab.units Histogram", xlab = "log(Ab.units)")
hist(log(std.raw[std.raw$Specie=="Pviv",]$Ab.unit), ylim = c(0,my), xlim = c(-6,10),
     main = "P.vivax", xlab = "log(Ab.units)")
hist(log(std.raw[std.raw$Specie=="Pfal",]$Ab.unit), ylim = c(0,my), xlim = c(-6,10),
...
#library(ggplot2)
#library(Rmisc)        #multiploting ggplots
a <- ggplot(std.raw, aes(x=mean.OD, fill=Specie)) + 
...
b <- ggplot(std.raw, aes(x=Ab.unit, fill=Specie)) + 
  geom_histogram(alpha=.5, position = "identity") + 
...
```{r}
std.uty <- data.frame()
for (i in 1:length(levels(std.nty$ID))) {
  finMAT <- data.frame(ID= levels(std.nty$ID)[i], 
                       Ab.unit_Pfal= 
                         if (identical(std.nty[std.nty$ID==levels(std.nty$ID)[i] & 
                                             std.nty$Specie==levels(std.nty$Specie)[1],4],
                                       numeric(0))) {
                           NaN
                         } else {
                           std.nty[std.nty$ID==levels(std.nty$ID)[i] & 
                                   std.nty$Specie==levels(std.nty$Specie)[1],4]
                         }
                       , 
                       Ab.unit_Pviv= 
                         if (identical(std.nty[std.nty$ID==levels(std.nty$ID)[i] & 
                                             std.nty$Specie==levels(std.nty$Specie)[2],4],
                                       numeric(0))) {
                           NaN
                         } else {
                           std.nty[std.nty$ID==levels(std.nty$ID)[i] & 
                       ...
                       mean.OD_Pfal= 
                         if (identical(std.nty[std.nty$ID==levels(std.nty$ID)[i] & 
                                             std.nty$Specie==levels(std.nty$Specie)[1],5],
                                       numeric(0))) {
                           NaN
                         } else {
                           std.nty[std.nty$ID==levels(std.nty$ID)[i] & 
                       ...
                       mean.OD_Pviv= 
                         if (identical(std.nty[std.nty$ID==levels(std.nty$ID)[i] & 
                                             std.nty$Specie==levels(std.nty$Specie)[2],5],
                                       numeric(0))) {
                           NaN
                         } else {
                           std.nty[std.nty$ID==levels(std.nty$ID)[i] & 
  ...
  std.uty <- rbind(std.uty,finMAT)
}
#colnames(std.uty) <- c("ID", "Ab.unit_Pfal", "Ab.unit_Pviv")
str(std.uty)
summary(std.uty)
```
