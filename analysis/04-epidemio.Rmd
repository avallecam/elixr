...
- The **109 variables** of the Epidemiological database were reduced to **88** after cleaning.
- typos were corrected:
    + some variables were renamed, and some factors recoded.
- previous datasets share **959 ID's** with the current Epidemiological dataset.
...
- **inconsistencies** between other variables are shown as table summaries.
...
- Merge **`unap + sero`** stored in `data/03-covariat-uty.csv` (`std.uty.cov`) with current **`epid`** (`unap6a`)
...
- **`epid`** and **`unap + sero`** differs in `age`, `sex`, `community`
    ...
    + logical vectors such as `_0` and `_n1` were removed from **`unap+sero`** previous merging
...
# APPENDIX: age updates

```{r, fig.height=8, fig.width=6, fig.align='center'}
x <- unap6a %>% 
  select(1,contains("age")) %>% 
  full_join(std.uty.cov %>% 
              select(1,contains("age"), -age_0), 
            by="id") %>% 
