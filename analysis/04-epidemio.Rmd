...
```{r}
unap6a <- unap6a %>% 
  mutate_at(vars(ah_radio:ah_auto), factor) %>% #select(1, contains("enf")) %>% glimpse()
  mutate(ah_radio=fct_recode(ah_radio,"si"="1","no"="0"), 
         ...
         ah_auto=fct_recode(ah_auto,"si"="1","no"="0")) #%>% select(enf_diabetes_si,enf_diabetes_no) %>% count()
...
unapid <- unap0 %>% select(id)      # 960
seroid <- std.uty %>% select(id=ID) # 969
epidid <- unap6a %>% select(id)     # 963
...
- Merge `data/03-covariat-uty.csv` and `unap6a`
- This is the final unified dataset.

dicoveries:

- `unap6a` and `std.uty.cov` differs in `age`, `sex`, `community`
- `age`, `sex`, `community` were removed from `unap6a` 
- then it was merged with `std.uty.cov`
- logics such as `_0` and `_n1` were removed from `std.uty.cov` previously

```{r}
unap6b <- unap6a %>% 
  select(-age, -sex, -community) %>% 
...
```{r}
readr::write_csv(unap6b, "data/04-epidemio-fll.csv")
saveRDS(unap6b, "data/04-epidemio-fll.rds")

readr::write_csv(unap6c, "data/04-epidemio-inn.csv")
saveRDS(unap6c, "data/04-epidemio-inn.rds")
```
