...
unap6a <- unap6a %>% #str()
  ...
  #select(enf_diabetes_si,enf_diabetes_no) %>% count()
  ...
  #select(enf_neurologica_si,enf_neurologica_no) %>% count()
  ...
  #select(enf_pulmonar_si,enf_pulmonar_no) %>% count()
  ...
  #select(enf_cardiaca_si,enf_cardiaca_no) %>% count()
  ...
  #select(enf_renal_si,enf_renal_no) %>% count()
...
data %>% mutate_if(is.character, factor) %>% glimpse()
data %>% mutate_if(is.character, factor) %>% summary()
```
