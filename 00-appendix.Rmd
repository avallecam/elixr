...
All the reports are written in **R** [@R] within a `Rmarkdown` [@rmarkdown] notebook 
using the `RStudio` [@rstudio] IDE software, and employing the `knitr` [@knitr] 
and `Hmisc` [@Hmisc] packages for the `html` setup.
...
* Principal: 
    - Standardize the OD~450nm~ values across ELISA plates by the estimation of 
    the Antibody Units of each unknown sample.

* Secondary:
    - Implement a [reproducible workflow](https://www.ncbi.nlm.nih.gov/pubmed/26776185) 
    for standardization of ELISA plates from a template matrix.
...
1. An **endpoint titer** equal to the last dilution of the STD, in this case 
`r format(x[length(x)], scientific=FALSE)`.
...
- For a comparison between resting NaN's mean.OD and Upper/Lower limits per ELISA plate:
    ...
    [02-dtfilter.html](report/02-dtfilter.html#appendix-residual-na)
    + Complete list of Upper/Lower limits per ELISA plate in 
    [01-standard.html](report/01-standard.html#appendix-upperlower-limits)
...
[02-dtfilter.html](report/02-dtfilter.html#db-typo-corrections):
    + 2 samples were added: ZG009-3 and NN068-2.

- Complete list of replicates before and after the unbiased selection available at 
[02-dtfilter.html](report/02-dtfilter.html#appendix-replicates)

- Covariate DB was added to the analysis at 
[03-covariat.html](report/03-covariat.html#covariates):
