...
z0db <- read_dta("data/z0_ind_viv_t3.dta") %>% 
  ...
  mutate(sero_viv=as.numeric(sero_viv)-1)
...
glm.null <- glm(sero_viv ~ 1, data = z0db_cc, family = poisson(link = "log"))
...
add1(update(glm.null, ~ . + age_quart),scope = myformula,test = "LRT") %>% 
  epi_tidynested(2) -> rank_l2
add1(update(glm.null, ~ . + age_quart + recibio_tratamiento_malaria_c),scope = myformula,test = "LRT") %>% 
  epi_tidynested(3) -> rank_l3
add1(update(glm.null, ~ . + age_quart + recibio_tratamiento_malaria_c + epi_uso_red_dormir_c),scope = myformula,test = "LRT") %>% 
  epi_tidynested(4) -> rank_l4
add1(update(glm.null, ~ . + age_quart + recibio_tratamiento_malaria_c + epi_uso_red_dormir_c + prev_viv),scope = myformula,test = "LRT") %>% 
  epi_tidynested(5) -> rank_l5
add1(update(glm.null, ~ . + age_quart + recibio_tratamiento_malaria_c + epi_uso_red_dormir_c + prev_viv + ah_mototaxi),scope = myformula,test = "LRT") %>% 
  epi_tidynested(6) -> rank_l6
add1(update(glm.null, ~ . + age_quart + recibio_tratamiento_malaria_c + epi_uso_red_dormir_c + prev_viv + ah_mototaxi + ah_motocicleta),scope = myformula,test = "LRT") %>% 
  epi_tidynested(7) -> rank_l7
#add1(update(glm.null, ~ . + age_quart + recibio_tratamiento_malaria_c + epi_uso_red_dormir_c + prev_viv + ah_mototaxi + ah_motocicleta + age_7),scope = myformula,test = "LRT") %>% 
#  epi_tidynested(7) -> rank_l8

# parsimonius model -------------------------------------------------------------

wm1 <- update(glm.null, ~ . + age_quart + recibio_tratamiento_malaria_c + epi_uso_red_dormir_c + prev_viv + ah_mototaxi + ah_motocicleta)
...
CGPfunctions::newggslopegraph(rank_table_term_l,rank,value,term,Title = "Niveles modelo anidado",SubTitle = "Y: SERO+ P. vivax")
ggsave("figure/slopegraph-nested-serov.png",width = 12,height = 8)
...
final %>% print(n=Inf)
final %>% as.data.frame() %>% 
  xlsx::write.xlsx("table/z0-nested-r_serov.xlsx",showNA = FALSE)
